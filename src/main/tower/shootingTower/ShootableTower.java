package main.tower.shootingTower;
...
import main.tower.Attack;
import main.tower.LevelOfTower;
...
public class ShootableTower extends Tower {
    ...
    private int kills = 0;
    private STATE state = STATE.Shootable;

    public ShootableTower(ArrayList<Placeable> allObjects, int difficulty, double framerate, int x, int y,
                          ...
                          int price) {
        super(allObjects, STATE.Shootable, x, y, dimension, colourOfTower, shape, price, difficulty);
     ...
     * if it can then which enemy it can shoot and then informs the enemy that it has been shoot.
     */
    public void tick(EnemyWave allEnemies) { // TODO Move to shootingAction so that we have only data in this class!
        ...
        if (attack.canShootAtThisFrame()) {
            ...
            for (Enemies currentEnemy : allEnemies ) { // loop through all enemy
   ...
   public Enemies getLastTarget() {
    ...
    /**
     * CAUTION this replaces the current shootingAction with a new one in ShootableTower!
     * @param action
     */
    @Override
